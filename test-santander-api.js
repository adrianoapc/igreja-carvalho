...
async function getStatement(
  ...
  const url = new URL(
    `${SANTANDER_BASE_URL}/banks/${bankId}/statements/${agency}.${account}`
  );

  // Adicionar parâmetros
  if (options.initialDate)
    url.searchParams.set("initialDate", options.initialDate);
  ...
  try {
    ...
    if (response.status !== 200) {
      ...
      logInfo(`Resposta: ${response.raw}`);
      throw new Error(
        `Statement request failed with status ${response.status}`
      );
    }
    ...
    if (Array.isArray(response.body)) {
      ...
      if (response.body.length > 0) {
        logInfo(
          `Primeira transação: ${JSON.stringify(response.body[0], null, 2)}`
        );
      }
    }
