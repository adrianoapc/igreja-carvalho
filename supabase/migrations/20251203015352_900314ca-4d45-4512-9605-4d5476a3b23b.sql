-- Add sublider_id column to times_culto table
ALTER TABLE public.times_culto 
ADD COLUMN sublider_id uuid REFERENCES public.profiles(id);

-- Create index for sublider_id
CREATE INDEX idx_times_culto_sublider_id ON public.times_culto(sublider_id);

-- Update the get_minha_lista_chamada function to include teams where user is leader OR sublider
