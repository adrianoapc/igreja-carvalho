-- Fix: Rejeitar todas as sugestões que envolvem os mesmos extratos e transações
-- Quando usuário rejeita uma sugestão, não faz sentido continuar sugerindo a mesma combinação
...
BEGIN
  ...
  -- Se o usuário rejeitou essa combinação específica, não deve ver outras sugestões iguais
  SELECT ARRAY_AGG(id) INTO v_sugestoes_relacionadas
  FROM conciliacao_ml_sugestoes
  WHERE status = 'pendente'
    AND igreja_id = v_sugestao.igreja_id
    AND extrato_ids = v_sugestao.extrato_ids  -- Exatamente os mesmos extratos
    AND transacao_ids = v_sugestao.transacao_ids;  -- Exatamente as mesmas transações
  ...
  -- (evitar duplicar feedback para todas as 199 sugestões)
