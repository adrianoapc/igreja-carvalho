-- Corrigir função get_user_auth_context para usar user_id ao invés de id na busca do profile
...
BEGIN
  -- 1. Buscar profile do usuário (CORRIGIDO: user_id ao invés de id)
  SELECT id, user_id, igreja_id, filial_id, nome
  INTO v_profile
  FROM profiles
  ...
  -- 2. Buscar roles do usuário na igreja (p_user_id = auth.users.id)
  ...
  -- 8. Montar resultado final (retorna user_id como id para consistência com auth.uid())
...
CORRIGIDO: Usa WHERE user_id = p_user_id (FK para auth.users) ao invés de WHERE id = p_user_id.';