...
-- Função auxiliar: validação de integridade de transferências
...
AS $$
BEGIN
  -- Transferência sem transação de entrada
  RETURN QUERY
  SELECT 
  ...
  -- Transferência sem transação de saída
  RETURN QUERY
  SELECT 
  ...
  -- Entrada e saída com valores diferentes
  RETURN QUERY
  SELECT 
