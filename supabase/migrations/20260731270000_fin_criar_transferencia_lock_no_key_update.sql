-- ============================================================================
-- 1 achado do /code-review (PR #67, rodada de 01/08 22:31, commit revisado
-- 3d89e70), real:
--
-- fin_criar_transferencia (20260731100000) trava as duas contas em ordem
-- determinística ANTES de inserir (evita deadlock entre 2 transferências
-- concorrentes em direções opostas — achado de rodada anterior) usando
-- `FOR UPDATE`. Mas os 2 INSERTs pagos logo depois (na mesma transação)
-- disparam o trigger de saldo, que precisa gravar em `contas` — e
-- qualquer INSERT em `transacoes_financeiras` já retém um lock `KEY SHARE`
-- na linha de `contas` referenciada (checagem da FK `conta_id`) até
-- commitar. `FOR UPDATE` conflita com `KEY SHARE` de OUTRA sessão: um
-- INSERT pago comum rodando concorrentemente numa dessas contas (fora da
-- transferência) já detém o `KEY SHARE`; a transferência tentando `FOR
-- UPDATE` na mesma linha espera, e se esse INSERT concorrente por sua vez
-- também espera algo que a transferência seinternamente. mesma classe de
-- bug já corrigida em `_fin_recalcular_saldo_conta_raw` (20260731200000) e
-- em `fin_recalcular_saldo_conta` (20260730110000) — aqui só não tinha
-- sido replicado ainda porque o `FOR UPDATE` desta função resolve um
-- problema DIFERENTE (ordem determinística entre 2 contas, não
-- serialização de recálculo de saldo de 1 conta), mas o lock em si sofre
-- do mesmo conflito com o `KEY SHARE` do trigger.
--
-- Fix: `FOR NO KEY UPDATE` em vez de `FOR UPDATE` — continua serializando
-- as duas transferências concorrentes na mesma ordem (o que evita o
-- deadlock original entre elas), mas sem brigar com o `KEY SHARE` que um
-- INSERT pago comum (fora da transferência) já detém nessas contas.
-- ============================================================================
...
BEGIN
  ...
  -- concorrentes em direções opostas (A->B e B->A). FOR NO KEY UPDATE, não
  -- FOR UPDATE: os 2 INSERTs pagos logo abaixo disparam o trigger de saldo
  -- (grava em contas), e qualquer INSERT em transacoes_financeiras (aqui
  -- ou em outra sessão concorrente) já retém KEY SHARE nessas linhas de
  -- contas via checagem de FK — FOR UPDATE conflitaria com esse KEY SHARE,
  -- reabrindo risco de deadlock com um INSERT pago comum concorrente fora
  -- da transferência (achado do /code-review; mesmo padrão já corrigido em
  -- _fin_recalcular_saldo_conta_raw e fin_recalcular_saldo_conta). FOR NO
  -- KEY UPDATE continua serializando as duas transferências entre si (o
  -- problema original que este lock resolve) sem brigar com KEY SHARE.
