-- Achado ao usar o fluxo real de antecipação Getnet em produção
-- (2026-08-13): a tela "Conciliação Cartão" (Fase 7a/7b,
-- ConciliacaoCartaoLedger.tsx) não oferece nenhuma ação de reverter um
-- lote 'lancamento_criado' — só mostra "Concluído", sem botão. O caminho
-- existente (menu de transações > "Reverter pagamento") fica fora desta
-- tela, obrigando o usuário a sair daqui pra achar o lançamento em outro
-- lugar. A RPC deste ledger nunca devolveu `lancamento_desagio_id`, então
-- o frontend não tinha como saber QUAL transação reverter mesmo se
-- quisesse oferecer o botão aqui.
--
-- Fix: expõe `lancamento_desagio_id` no jsonb de cada lote — único ajuste
-- de verdade. Resto do corpo idêntico a 20260807120000 (fonte confirmada
-- via `grep -rl "CREATE OR REPLACE FUNCTION public.fin_listar_ledger_
-- conciliacao_cartao" supabase/migrations/*.sql | sort`, guardrail 6b/
-- docs/guardrails-financeiro.md — corpo inteiro via CREATE OR REPLACE,
-- não dá pra só editar o arquivo histórico).
  ...
  lotes_final AS (
    SELECT jsonb_agg(
             jsonb_build_object(
               ...
               -- Novo (2026-08-13): expõe o lançamento de deságio pro
               -- frontend oferecer "Reverter" direto nesta tela — antes
               -- só existia via TransacaoActionsMenu, fora deste ledger.
               'lancamento_desagio_id', lc.lancamento_desagio_id,
