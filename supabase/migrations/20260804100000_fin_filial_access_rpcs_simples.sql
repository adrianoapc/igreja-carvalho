-- ============================================================================
-- has_filial_access nas 4 RPCs mais simples ainda sem o check (docs/
-- arquitetura-financeiro.md §11): fin_desconciliar, fin_alterar_status_
-- lancamento, fin_alternar_conferencia_manual, fin_marcar_extrato_ignorado.
--
-- Mesmo bug em todas: cada uma já lê (ou tem disponível de graça) a
-- `filial_id` do recurso que opera (transação ou extrato), mas nunca chama
-- `has_filial_access` antes de agir — um tesoureiro restrito a UMA filial
-- consegue desconciliar/alterar status/conferir manualmente/ignorar extrato
-- de QUALQUER filial do tenant.
--
-- Fix: mesmo bloco já validado em fin_atualizar_lancamento/fin_excluir_
-- lancamento (PR #67), inserido logo após o SELECT ... FOR UPDATE que trava
-- a linha, antes de qualquer validação de negócio:
--   IF NOT public.has_filial_access(v_igreja, <filial_do_recurso>) THEN
--     RAISE EXCEPTION 'FIN_TENANT: sem acesso à filial ...';
--   END IF;
--
-- Testado em harness Postgres standalone (postgres:15, isolado): 16
-- cenários (4 por RPC — rejeita filial cruzada / aceita própria filial /
-- admin sempre passa / JWT sem filial passa, comportamento documentado de
-- has_filial_access), todos verdes.
-- ============================================================================
...
BEGIN
  ...
  IF NOT public.has_filial_access(v_igreja, v_trx.filial_id) THEN
    RAISE EXCEPTION 'FIN_TENANT: sem acesso à filial desta transação';
  END IF;
