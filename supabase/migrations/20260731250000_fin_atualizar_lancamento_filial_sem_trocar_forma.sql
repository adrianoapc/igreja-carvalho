-- ============================================================================
-- 1 achado do /code-review (PR #67, rodada de 01/08 22:01, commit revisado
-- cfb8b18), real — efeito colateral do meu próprio guard de filial de
-- forma_pagamento_id em fin_atualizar_lancamento (§9.62/§9.63):
--
-- O guard só rodava `IF v_aplicar ? 'forma_pagamento_id'` — ou seja, só
-- quando o PATCH mexe explicitamente na forma de pagamento. Mas
-- TransacaoDialog.tsx SEMPRE inclui `filial_id` no patch (`isAllFiliais ?
-- null : filialId`, campo de `camposComuns`), e só inclui `forma_pagamento_
-- id` quando o usuário realmente troca a forma (`formaPagamentoMudou`).
-- Editar uma transação de uma filial específica a partir da visão "Todas
-- as filiais" SEM tocar na forma de pagamento manda `filial_id: null`
-- (converte pra global) mas OMITE `forma_pagamento_id` — o backend mantém
-- a forma_pagamento_id já gravada (ainda presa à filial original) enquanto
-- a transação vira global. Resultado: exatamente o problema que o guard de
-- §9.62 existe pra evitar (transação global referenciando metadado privado
-- de uma filial, invisível via RLS pra quem está em outra), só que
-- alcançado por um caminho que o guard não cobria — mudar SÓ filial_id.
--
-- Fix: o guard passa a rodar sempre que `forma_pagamento_id` OU `filial_id`
-- estiver no patch, validando o par EFETIVO (o valor do patch quando
-- presente, senão o já gravado) dos dois — não só quando os dois mudam
-- juntos.
-- ============================================================================
...
BEGIN
  ...
  -- §9.64: valida o PAR EFETIVO forma_pagamento_id/filial_id — o valor do
  -- patch quando presente, senão o já gravado — sempre que qualquer um dos
  -- dois estiver no patch, não só quando os dois mudam juntos. Sem isso,
  -- mudar só filial_id (ex.: mover pra "Todas as filiais" sem tocar na
  -- forma de pagamento — é exatamente o que TransacaoDialog.tsx faz)
  -- reconvertia a transação em global mantendo uma forma_pagamento_id
  -- ainda presa a uma filial específica (achado do /code-review).
  IF v_aplicar ? 'forma_pagamento_id' OR v_aplicar ? 'filial_id' THEN
    v_forma_id_efetivo := CASE WHEN v_aplicar ? 'forma_pagamento_id'
                                THEN NULLIF(v_aplicar ->> 'forma_pagamento_id','')::uuid
                                ELSE v_atual.forma_pagamento_id END;
    v_filial_efetiva := CASE WHEN v_aplicar ? 'filial_id'
                              THEN NULLIF(v_aplicar ->> 'filial_id','')::uuid
                              ELSE v_atual.filial_id END;
    IF v_forma_id_efetivo IS NOT NULL THEN
      SELECT filial_id INTO v_forma_filial FROM public.formas_pagamento WHERE id = v_forma_id_efetivo;
...
COMMENT ON FUNCTION public.fin_atualizar_lancamento(uuid, jsonb, jsonb) IS
  'Atualiza lançamento (patch parcial): D4 bloqueia conciliado, D10 bloqueia divergir competência de parcela isolada num grupo parcelado, forma_pagamento_id (FK real, ADR-029) validado via fin_validar_fk_tenant + filial (par efetivo forma/filial, mesmo quando só um dos dois muda) e sincronizado com o texto legado forma_pagamento.';
