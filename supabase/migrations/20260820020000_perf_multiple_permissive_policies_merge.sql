...
--
-- 2ª RODADA (mesmo dia): review do Codex/Cursor achou que `profiles`,
-- `inscricoes_eventos`, `eventos_convites`, `familias`, `fornecedores`
-- e `visitante_contatos` têm policies "ver"/"criar" que já são `FOR
-- ALL` HOJE em produção (bug pré-existente — confirmado no dump bruto,
-- sem relação com esta PR; `escalas` já estava excluída desde a 1ª
-- rodada). O merge não alarga matematicamente nada além do que já vale
-- (Postgres já faz OR entre policies PERMISSIVE aplicáveis, mescladas
-- ou não), mas consolidar um bug pré-existente numa policy nova
-- `perf_merge_*` piora a rastreabilidade dele — mesmo raciocínio da 1ª
-- rodada. Estas 6 tabelas foram REMOVIDAS desta migration também. Ver
-- memória project-policies-for-clause-perdido-pendente para o fix de
-- fundo (restaurar o `FOR` correto por tabela).
