...
-- Pais podem ver diários dos próprios filhos (via tabela familias)
CREATE POLICY "Pais veem diarios dos filhos" ON public.kids_diario
  FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM public.familias f
      WHERE f.familiar_id = kids_diario.crianca_id
      AND f.pessoa_id IN (SELECT id FROM public.profiles WHERE user_id = auth.uid())
    )
  );

-- Trigger para atualizar updated_at (usando função existente)
CREATE TRIGGER update_kids_diario_updated_at
  BEFORE UPDATE ON public.kids_diario
  FOR EACH ROW
  EXECUTE FUNCTION public.update_updated_at_column();

-- View para facilitar consultas com dados joinados (SECURITY INVOKER)
CREATE OR REPLACE VIEW public.view_kids_diario 
