-- ============================================================================
-- Fix — fin_listar_ledger_conciliacao_cartao inclui lançamentos já fechados
-- por outro canal (conciliacao_status='conciliado_extrato') como "sem_hop2".
--
-- Reproduzido com dado real de produção: ledger da Fase 7 carrega OK (hotfix
-- de drift de `parcelas` de 20260807100000 funcionou), mas clicar "Buscar
-- manualmente" numa linha classificada como sem_hop2 ("Pendente vínculo")
-- disparava `fin_buscar_recebiveis_getnet_oferta` → erro
-- `FIN_JA_LANCADO: lançamento já conciliado (conciliado_extrato); não há
-- recebíveis para buscar`.
--
-- Causa: a CTE `raizes` (20260806100000, repetida sem mudança em
-- 20260807100000) filtra transacoes_financeiras por tipo/status/forma
-- ILIKE '%cart%'/tenant/filial, mas NUNCA olhou `conciliacao_status`. Isso
-- inclui lançamentos que já foram conciliados por um canal totalmente
-- diferente do ledger de cartão — batidos direto contra o extrato bancário
-- (Modo Inteligente / conciliação manual de extrato), sem nunca passar pela
-- cadeia Getnet Hop1/Hop2. Como esse lançamento nunca ganha uma linha em
-- getnet_recebivel_lancamentos, `recebiveis` fica vazio → status vira
-- sem_hop2 → UI mostra "Pendente vínculo" com botões "Buscar
-- manualmente"/"Confirmar sugestão" que SEMPRE falham, porque o lançamento
-- já está fechado (só que por outro caminho, e fin_buscar_recebiveis_
-- getnet_oferta corretamente rejeita qualquer conciliacao_status distinto
-- de 'nao_conciliado').
--
-- Fix: exclui só 'conciliado_extrato' da CTE `raizes` — explícito via
-- `IN ('nao_conciliado', 'conciliado_manual')`, não
-- `<> 'conciliado_extrato'` (mais robusto se o enum ganhar um 4º valor no
-- futuro; falha fechado, não aberto).
--
-- Por que NÃO filtrar por conciliacao_status = 'nao_conciliado' (fix
-- errado, cogitado e descartado): quebraria as linhas fechado/aguardando_
-- banco/divergencia, que dependem de lançamentos com
-- conciliacao_status='conciliado_manual' — setado por
-- fin_vincular_venda_getnet_oferta quando o Hop 2 via Getnet É confirmado
-- (ver 20260805110000/20260807100000). Esse status TEM que continuar
-- aparecendo no ledger; só 'conciliado_extrato' significa "fechado por um
-- canal que não é este ledger".
--
-- Enum REAL de transacoes_financeiras.conciliacao_status (CHECK constraint,
-- confirmado no harness Postgres, não só por leitura do briefing original) —
-- 4 valores, não 3: nao_conciliado | conciliado_manual | conciliado_extrato |
-- conciliado_bot (adicionado em 20260213201114, nenhuma RPC deste repo
-- escreve esse valor hoje — provavelmente reservado pra um canal de
-- conciliação automática ainda não implementado — mas ~15 RPCs `fin_*` já
-- tratam `conciliado_bot` como sinônimo de "fechado por outro canal",
-- sempre no idioma `conciliacao_status IN ('conciliado_extrato',
-- 'conciliado_bot')` como guard de "não pode reeditar" — ver
-- fin_atualizar_lancamento, fin_saldo_conta_sempre_recalcula,
-- fin_conferencia_form_orfao_reparent, fin_excluir_lancamento_filial_access
-- etc. Esse idioma já existente no codebase é o motivo pelo qual o allow-
-- list explícito abaixo (`IN ('nao_conciliado', 'conciliado_manual')`) é o
-- fix certo e não `<> 'conciliado_extrato'`: um deny-list só do
-- 'conciliado_extrato' deixaria `conciliado_bot` passar pela CTE `raizes`
-- do mesmo jeito que o bug original deixava `conciliado_extrato` passar —
-- mesma classe de bug, só que no valor que a enumeração 3-valores do
-- briefing original não previa.
--
-- Migration NOVA (forward), corpo inteiro da função via CREATE OR REPLACE —
-- guardrail 6b (docs/guardrails-financeiro.md): fin_listar_ledger_
-- conciliacao_cartao já está deployada em produção desde 20260807100000;
-- editar esse arquivo histórico no git não reescreve o corpo no próximo
-- `supabase db push` (só roda arquivos ainda não aplicados). Único ajuste
-- de verdade nesta migration: 1 linha nova no WHERE da CTE `raizes`, abaixo.
-- Resto do corpo é idêntico a 20260807100000 (fonte confirmada como a
-- versão mais recente via
-- `grep -rl "CREATE OR REPLACE FUNCTION public.fin_listar_ledger_conciliacao_cartao" supabase/migrations/*.sql | sort`
-- — guardrail 6 do mesmo doc).
-- ============================================================================
