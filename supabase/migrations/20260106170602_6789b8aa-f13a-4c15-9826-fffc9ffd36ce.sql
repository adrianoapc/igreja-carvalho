-- Atualiza função para suportar filtro por filial
CREATE OR REPLACE FUNCTION public.get_ovelhas_em_risco(
  p_filial_id UUID,
  p_is_all BOOLEAN DEFAULT FALSE
)
RETURNS TABLE (
  id UUID,
  nome TEXT,
...
BEGIN
  ...
  -- 1. Risco por Ausência (Quem não vem há 21 dias) filtrado por filial quando aplicável
  SELECT
      p.id,
      p.nome,
      p.avatar_url,
  ...
  LEFT JOIN public.checkins pc ON pc.pessoa_id = p.id
      AND pc.igreja_id = current_igreja_id
      AND (p_is_all OR pc.filial_id = p_filial_id)
  WHERE p.status = 'membro'
    AND p.igreja_id = current_igreja_id
    AND (p_is_all OR p.filial_id = p_filial_id)
  ...
  -- 2. Risco por Sentimento (Registrou tristeza nos últimos 7 dias) filtrado por filial quando aplicável
  SELECT
      p.id,
      p.nome,
      p.avatar_url,
  ...
  WHERE sm.data_registro > (now() - INTERVAL '7 days')
    ...
    AND p.igreja_id = current_igreja_id
    AND (p_is_all OR sm.filial_id = p_filial_id)
    AND (p_is_all OR p.filial_id = p_filial_id)
...
COMMENT ON FUNCTION public.get_ovelhas_em_risco(UUID, BOOLEAN) IS 'Retorna membros em risco pastoral com filtro por filial opcional: ausentes há mais de 21 dias ou com sentimentos negativos recentes';