...
const SANTANDER_TOKEN_URL =
  "https://trust-open.api.santander.com.br/auth/oauth/v2/token";
const SANTANDER_BASE_URL =
...
function decryptData(encrypted: unknown, key: Uint8Array): string {
  ...
  try {
    ...
    if (combined.length < 25) {
      console.log(
        "[test-santander] Data too short to be encrypted, returning as-is"
      );
      return encryptedStr;
    }
    ...
    console.warn(
      "[test-santander] Base64 decode failed, returning original:",
      error
    );
    return encryptedStr;
  }
  ...
  try {
    ...
    if (pfxData.trim().startsWith("{")) {
      try {
        ...
        throw new Error(
          `Invalid JSON PFX: ${err instanceof Error ? err.message : String(err)}`
        );
      }
      ...
      try {
        pfxDer = forge.util.decode64(
          btoa(unescape(encodeURIComponent(pfxData)))
        );
      } catch {
    ...
    throw new Error(
      `PFX conversion failed: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { integracao_id, banco_id, agencia, conta, data_inicio, data_fim } =
      payload;
    ...
    if (!integracao_id || !banco_id || !agencia || !conta) {
      return jsonResponse(
        {
          error:
            "Missing required fields: integracao_id, banco_id, agencia, conta",
        },
        400
      );
    }
    ...
    console.log(
      "[test-santander] Starting test for integração:",
      integracao_id
    );
    ...
    if (integracao.provedor !== "santander") {
      return jsonResponse(
        { error: "Esta função é apenas para Santander" },
        400
      );
    }
    ...
    console.log(
      "[test-santander] client_id preview:",
      secrets.client_id?.substring(0, 30)
    );
    console.log(
      "[test-santander] client_id length:",
      secrets.client_id?.length
    );
    ...
    try {
      ...
      if (secrets.application_key) {
        console.log(
          "[test-santander] Attempting to decrypt application_key..."
        );
    ...
    if (!clientId || !clientSecret) {
      return jsonResponse(
        { error: "Client ID or Secret not found in decrypted data" },
        400
      );
    }
    ...
    console.log(
      "[test-santander] application_key length:",
    ...
    if (pfxBlob && pfxPassword) {
      try {
        ...
        console.log(
          `[test-santander] PFX blob preview: ${pfxBlob.substring(0, 100)}`
        );
        console.log(
          `[test-santander] PFX password: ${pfxPassword?.substring(0, 50)}...`
        );
        ...
        return jsonResponse(
          {
            ...
            detail:
              pfxError instanceof Error ? pfxError.message : String(pfxError),
          },
          500
        );
      }
      ...
      console.warn(
        "[test-santander] No PFX certificate found - will attempt standard TLS only"
      );
      console.warn(
        `[test-santander] pfxBlob: ${pfxBlob ? "present" : "null"}, pfxPassword: ${pfxPassword ? "present" : "null"}`
      );
    }
    ...
    if (!tokenResponse.ok) {
      ...
      console.error(
        "[test-santander] Token request failed:",
    ...
    if (!tokenData.access_token) {
      ...
      return jsonResponse(
        { error: "No access_token in response", detail: tokenData },
        400
      );
    }
    ...
    if (balanceResponse.ok) {
      ...
      console.warn(
        "[test-santander] Balance query failed:",
    ...
    statementUrl.searchParams.set(
      "initialDate",
      data_inicio || formatDate(thirtyDaysAgo)
    );
    ...
    if (statementResponse.ok) {
      ...
      console.log(
        "[test-santander] Statement query successful, got",
      ...
      console.warn(
        "[test-santander] Statement query failed:",
        statementResponse.status
      );
      statementError = {
    ...
    return jsonResponse({
      ...
      balance: balance
        ? { obtained: true, data: balance }
        : { obtained: false, error: balanceError },
      statement: statement
        ? {
            obtained: true,
