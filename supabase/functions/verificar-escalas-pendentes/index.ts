...
serve(async (req) => {
  ...
  try {
    ...
    const processIgreja = async (igrejaId: string) => {
      ...
      console.log(`📅 Buscando cultos entre ${em24h.toISOString()} e ${em48h.toISOString()} (igreja ${igrejaId})`);
      ...
      console.log(`📋 Encontradas ${escalasValidas.length} escala(s) pendente(s) elegíveis para lembrete (igreja ${igrejaId})`);
      ...
      console.log(`\n✨ Verificação concluída: ${lembretes_enviados} lembrete(s) enviado(s) (igreja ${igrejaId})`);
    ...
    for (const igrejaId of igrejaIds) {
      const resultado = await processIgreja(igrejaId);
      resultados.push(resultado);
      totalLembretes += resultado.lembretes_enviados ?? 0;
    }
