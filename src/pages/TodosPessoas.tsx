  ...
  const fetchPessoas = async () => {
    try {
      const { data, error } = await supabase
        .from("profiles")
        .select("id, nome, telefone, email, status, data_primeira_visita, numero_visitas, user_id")
        .order("nome");
                    ...
                    <div className="flex flex-col gap-1 mt-1">
                      ...
                      {pessoa.data_primeira_visita && (
                        <span className="flex items-center gap-1 text-xs md:text-sm text-muted-foreground">
                          ...
                          Cadastrado em: {format(new Date(pessoa.data_primeira_visita), "dd/MM/yyyy", { locale: ptBR })}
                        </span>
                      )}
                    </div>
                  </div>
