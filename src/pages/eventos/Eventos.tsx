...
import { format, isAfter, isBefore, startOfMonth, endOfMonth, addDays } from "date-fns";
...
import { Plus, Calendar as CalendarIcon, Clock, MapPin, User, List, CalendarDays, ExternalLink, Search, Filter, MoreHorizontal, Eye, Edit, Trash2, TrendingUp, CalendarCheck, AlertCircle } from "lucide-react";
...
const TIPO_CONFIG = {
  CULTO: { label: "Culto", color: "bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400" },
  EVENTO: { label: "Evento", color: "bg-purple-100 text-purple-700 dark:bg-purple-900/20 dark:text-purple-400" },
  RELOGIO: { label: "Relógio", color: "bg-orange-100 text-orange-700 dark:bg-orange-900/20 dark:text-orange-400" },
  TAREFA: { label: "Tarefa", color: "bg-gray-100 text-gray-700 dark:bg-gray-900/20 dark:text-gray-400" },
  OUTRO: { label: "Outro", color: "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/20 dark:text-indigo-400" }
};
...
export default function Eventos() {
  ...
  const filteredEventos = eventos.filter((evento) => {
    ...
    const matchesDate = (!dateRange.from || !isBefore(new Date(evento.data_evento), dateRange.from)) &&
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            {kpis.proximoEvento ? (
              <div>
                <div className="text-2xl font-bold">{kpis.proximoEvento.titulo}</div>
                <p className="text-xs text-muted-foreground">
                  {format(new Date(kpis.proximoEvento.data_evento), "dd/MM/yyyy", { locale: ptBR })}
                </p>
              </div>
            ) : (
          ...
          {isMobile ? (
            ...
            <div className="space-y-3">
              {filteredEventos.map((evento) => {
                ...
                const statusConfig = STATUS_CONFIG[evento.status as keyof typeof STATUS_CONFIG] || STATUS_CONFIG.planejado;
                const tipoConfig = TIPO_CONFIG[evento.tipo as keyof typeof TIPO_CONFIG] || TIPO_CONFIG.OUTRO;
                    ...
                    <CardContent className="p-4">
                      <div className="flex items-start justify-between mb-3">
                        <div className="flex-1 min-w-0">
                          <h3 className="font-semibold text-lg truncate">{evento.titulo}</h3>
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => handleAbrirEvento(evento)}>
