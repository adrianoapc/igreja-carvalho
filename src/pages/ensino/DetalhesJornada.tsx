  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (etapaIds.length > 0) {
        ...
        concluidoPorAluno = ((presencas || []) as PresencaEtapa[]).reduce(
          (acc, presenca) => {
      ...
      const inscricoesComProgresso: InscricaoComProgresso[] = (
        (inscricoesData || []) as RawInscricao[]
      ).map((inscricao) => ({
        ...inscricao,
  ...
  const handleSearch = async (term: string) => {
    ...
    try {
      ...
      const filtrados = (data || []).filter(
        (pessoa) => !inscritosIds.has(pessoa.id)
      );
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent className="space-y-4">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {jornada.exibir_portal && (
                <Badge variant="outline">Visível no portal</Badge>
              )}
            </div>
            ...
            inscricoes.map((inscricao) => {
              const progresso =
                totalEtapas === 0
                  ? 0
                  : Math.round(
                      (inscricao.etapasConcluidas / totalEtapas) * 100
                    );
                ...
                >
                  ...
                  <div className="flex-1 md:max-w-md w-full">
                    <div className="flex justify-between text-xs text-muted-foreground mb-1">
                      <span>
                        {inscricao.etapasConcluidas}/{totalEtapas} etapas
                      </span>
                      <span>{progresso}%</span>
                    </div>
      ...
      <Dialog open={matriculaOpen} onOpenChange={setMatriculaOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Busque a pessoa e confirme a matrícula.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto">
              ...
              {searchResults.map((pessoa) => (
                ...
                >
                  ...
                  <Button
                    size="sm"
                    onClick={() => handleMatricular(pessoa.id)}
                    disabled={saving}
                  >
