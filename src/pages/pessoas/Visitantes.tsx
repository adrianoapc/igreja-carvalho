...
export default function Visitantes() {
  ...
  const fetchVisitantes = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from("profiles").select("*").in("status", ["visitante", "frequentador"]).order("data_primeira_visita", {
        ascending: false
      });
  ...
  return <div className="space-y-4 md:space-y-6 p-2 sm:p-0">
          ...
          <div className="space-y-3 md:space-y-4">
            {filteredVisitantes.map((visitante, index) => <div key={visitante.id} ref={index === filteredVisitantes.length - 1 ? loadMoreRef : null} className="flex flex-col gap-3 p-3 md:p-4 rounded-lg transition-colors bg-[#eff0cf]">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-col gap-1 mt-1">
                      {visitante.telefone && <span className="flex items-center gap-1 text-xs md:text-sm text-muted-foreground">
                          <Phone className="w-3 h-3 flex-shrink-0" />
                          {formatarTelefone(visitante.telefone)}
                        </span>}
                      {visitante.email && <span className="flex items-center gap-1 text-xs md:text-sm text-muted-foreground truncate">
                          <Mail className="w-3 h-3 flex-shrink-0" />
                          <span className="truncate">{visitante.email}</span>
                        </span>}
                      {visitante.data_primeira_visita && <span className="flex items-center gap-1 text-xs md:text-sm text-muted-foreground">
                          <Calendar className="w-3 h-3 flex-shrink-0" />
                          1ª visita: {format(new Date(visitante.data_primeira_visita), "dd/MM/yyyy", {
                      locale: ptBR
                    })}
                        </span>}
                      {visitante.data_ultima_visita && visitante.data_ultima_visita !== visitante.data_primeira_visita && <span className="flex items-center gap-1 text-xs md:text-sm text-muted-foreground">
                          <Calendar className="w-3 h-3 flex-shrink-0" />
                          Última: {format(new Date(visitante.data_ultima_visita), "dd/MM/yyyy", {
                      locale: ptBR
                    })}
                        </span>}
                    </div>
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {visitante.status === "visitante" && visitante.numero_visitas >= 2 && <Badge variant="outline" className="text-xs bg-accent/10 text-accent-foreground">
                      Próxima visita = Frequentador
                    </Badge>}
                  {visitante.aceitou_jesus && <Badge variant="outline" className="text-xs">
                      <Check className="w-3 h-3 mr-1" />
                      Aceitou Jesus
                    </Badge>}
                  {visitante.batizado && <Badge variant="outline" className="text-xs bg-primary/10">
                      <Check className="w-3 h-3 mr-1" />
                      Convertido
                    </Badge>}
                  {visitante.deseja_contato && <Badge variant="outline" className="text-xs">
                      <PhoneCall className="w-3 h-3 mr-1" />
                      Deseja contato
                    </Badge>}
                  {visitante.recebeu_brinde && <Badge variant="outline" className="text-xs">
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <Button variant="outline" size="sm" className="flex-1 text-xs md:text-sm" onClick={() => handleOpenDetails(visitante)}>
                    Detalhes
                  </Button>
                  <Button variant="outline" size="sm" className="flex-1 text-xs md:text-sm" onClick={() => handleAgendarContato(visitante)}>
                    <PhoneCall className="w-3 h-3 mr-1" />
                    Agendar Contato
                  </Button>
                </div>
            ...
            {!hasMore && filteredVisitantes.length > 0 && <div className="text-center py-4 text-sm text-muted-foreground">
                Todos os visitantes foram carregados
              </div>}
            
            {filteredVisitantes.length === 0 && !isLoading && <div className="text-center py-8 text-sm text-muted-foreground">
      ...
      {selectedVisitante && <AgendarContatoDialog open={agendarContatoOpen} onOpenChange={setAgendarContatoOpen} visitanteId={selectedVisitante.id} visitanteNome={selectedVisitante.nome} onSuccess={fetchVisitantes} />}
    </div>;
}