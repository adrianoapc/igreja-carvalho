...
export default function ContatosDashboard() {
  ...
  const fetchContatos = async () => {
    try {
      ...
      const {
        data,
        error
      } = await supabase.from("visitante_contatos").select(`
          id,
          data_contato,
          tipo_contato,
          status,
        ...
        `).gte("data_contato", hoje.toISOString()).in("status", ["agendado", "pendente"]).order("data_contato", {
        ascending: true
      });
  ...
  const marcarComoRealizado = async (contatoId: string) => {
    try {
      const {
        error
      } = await supabase.from("visitante_contatos").update({
        status: "realizado"
      }).eq("id", contatoId);
      if (error) throw error;
      toast({
  ...
  const renderContato = (contato: Contato) => <div key={contato.id} className="p-3 md:p-4 rounded-lg transition-colors space-y-3 bg-[#eff0cf]">
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 space-y-2 min-w-0">
          ...
          <div className="space-y-1 text-xs md:text-sm">
            {contato.visitante.telefone && <p className="flex items-center gap-2 text-muted-foreground">
                <Phone className="w-3 h-3 flex-shrink-0" />
                {contato.visitante.telefone}
              </p>}
            {contato.visitante.email && <p className="flex items-center gap-2 text-muted-foreground truncate">
                <Mail className="w-3 h-3 flex-shrink-0" />
                <span className="truncate">{contato.visitante.email}</span>
              </p>}
            <p className="flex items-center gap-2 text-muted-foreground">
              <Clock className="w-3 h-3 flex-shrink-0" />
              {format(parseISO(contato.data_contato), "dd/MM/yyyy 'às' HH:mm", {
              locale: ptBR
            })}
            </p>
          </div>

          {contato.observacoes && <p className="text-xs md:text-sm text-muted-foreground italic line-clamp-2">
              {contato.observacoes}
            </p>}

          <div className="flex flex-wrap gap-2">
            {contato.tipo_contato && <Badge variant="outline" className="text-xs">
                {contato.tipo_contato}
              </Badge>}
            {contato.status && <Badge variant="outline" className="text-xs">
                {contato.status}
              </Badge>}
          </div>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-2">
        <Button size="sm" variant="outline" className="flex-1 text-xs md:text-sm" onClick={() => marcarComoRealizado(contato.id)}>
          <CheckCircle className="w-3 h-3 mr-1" />
          Realizado
        </Button>
        <Button size="sm" variant="outline" className="flex-1 text-xs md:text-sm" onClick={() => marcarComoNaoRealizado(contato.id)}>
  ...
  return <div className="space-y-4 md:space-y-6 p-2 sm:p-0">
      ...
      {contatos.length === 0 ? <Card>
          ...
          {amanha.length > 0 && <Card className="shadow-soft">
              <CardHeader className="p-4 md:p-6">
          ...
          {proximos.length > 0 && <Card className="shadow-soft">
              <CardHeader className="p-4 md:p-6">
                <CardTitle className="flex items-center gap-2 text-base md:text-lg">
                  <Calendar className="w-4 h-4 md:w-5 md:h-5" />
