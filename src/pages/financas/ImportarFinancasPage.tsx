...
export default function ImportarFinancasPage() {
    ...
    try {
      ...
      const { data: jobData, error: jobError } = await supabase
        ...
        .insert({
          ...
          tipo,
          file_name: fileName,
          total_rows: rows.filter((_, i) => !excluded[i]).length,
          status: "processing",
      ...
      for (let start = 0; start < transacoes.length; start += chunkSize) {
        ...
        // inteiro, como acontecia com o INSERT único.
      ...
      const failureCount = errs.length;
      await supabase
        .from("import_jobs")
        .update({
          status: "completed",
          imported_rows: successCount,
          rejected_rows: failureCount,
  ...
  const undoImport = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/undo-import`,
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Erro ao desfazer importação");
      }
  ...
  const stepTitles = ["Upload", "Mapeamento", "Validação", "Confirmação"];

  return (
    <div className="p-4 md:p-6 space-y-6 min-h-screen max-w-6xl mx-auto">
      {/* Cabeçalho */}
      ...
      {/* Indicador de Steps (Wizard Style) */}
      ...
      <div className="space-y-4 min-h-96">
          ...
          <div className="space-y-4">
            ...
            {validationIssues.length === 0 ? (
              <Alert>
                ...
                <AlertDescription className="text-xs">
                  Nenhum erro encontrado. Pronto para importar.
                </AlertDescription>
              </Alert>
            ) : (
              <div className="space-y-3">
        ...
        {step === 3 && (
          <div className="space-y-3">
            <h3 className="font-semibold text-sm">Importação em Progresso</h3>
            <div className="space-y-2">
