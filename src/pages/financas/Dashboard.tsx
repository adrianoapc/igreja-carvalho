...
export default function Dashboard() {
  ...
  const receitasMesAtual = transacoesMesAtual?.filter(t => t.tipo === 'entrada').reduce((sum, t) => sum + Number(t.valor), 0) || 0;
  const despesasMesAtual = transacoesMesAtual?.filter(t => t.tipo === 'saida').reduce((sum, t) => sum + Number(t.valor), 0) || 0;
  const saldoMesAtual = receitasMesAtual - despesasMesAtual;

  // Cálculos do mês anterior
  const receitasMesAnterior = transacoesMesAnterior?.filter(t => t.tipo === 'entrada').reduce((sum, t) => sum + Number(t.valor), 0) || 0;
  const despesasMesAnterior = transacoesMesAnterior?.filter(t => t.tipo === 'saida').reduce((sum, t) => sum + Number(t.valor), 0) || 0;

  // Variações percentuais
  const variacaoReceitas = receitasMesAnterior > 0 ? ((receitasMesAtual - receitasMesAnterior) / receitasMesAnterior) * 100 : 0;
  const variacaoDespesas = despesasMesAnterior > 0 ? ((despesasMesAtual - despesasMesAnterior) / despesasMesAnterior) * 100 : 0;
  ...
  transacoesMesAtual?.forEach(t => {
    if (t.categoria) {
      ...
      const atual = categoriaMap.get(nome) || { nome, valor: 0, cor: t.categoria.cor };
      categoriaMap.set(nome, { ...atual, valor: atual.valor + Number(t.valor) });
    }
  });
        ...
        <Card className="shadow-soft">
          <CardHeader>
            <CardTitle className="text-lg flex items-center gap-2">
              ...
              Top 5 Categorias - {format(mesAtual, 'MMMM/yyyy', { locale: ptBR })}
            </CardTitle>
          </CardHeader>
          <CardContent>
                ...
                {dadosCategorias.map((cat, idx) => (
                  <div key={cat.nome} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">{formatCurrency(cat.valor)}</p>
                    </div>
                    <p className="text-sm font-semibold">
                      {((cat.valor / (receitasMesAtual + despesasMesAtual)) * 100).toFixed(1)}%
                    </p>
                  </div>
                ))}
              </div>
