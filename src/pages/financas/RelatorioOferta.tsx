...
export default function RelatorioOferta() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const temValores = Object.values(valores).some(
      ...
      if (conferente) {
        await supabase.from("notifications").insert({
          ...
          message: `${
            ...
            dataCulto,
            "dd/MM/yyyy"
          )} aguardando sua validação. Total: ${formatCurrency(
            calcularTotal()
          )}`,
  ...
  const handleConfirmarOferta = async (
    ...
    try {
      ...
      if (!contaOfertas || !contaSantander) {
        toast.error(
          'Contas "Ofertas" ou "Santander" não encontradas. Configure as contas primeiro.'
        );
        setLoading(false);
        return;
      }
      ...
      for (const [formaId, valorStr] of Object.entries(valoresMetadata)) {
        ...
        const isCartaoDebito =
          nomeLower.includes("débito") || nomeLower.includes("debito");
          ...
          <CardContent className="p-4 md:p-6 pt-0 space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {formasPagamento?.map((forma) => {
                const isCartaoCredito =
                  forma.nome.toLowerCase().includes("crédito") ||
                  forma.nome.toLowerCase().includes("credito");
                const isCartaoDebito =
                  forma.nome.toLowerCase().includes("débito") ||
                  forma.nome.toLowerCase().includes("debito");

                return (
                  <div key={forma.id} className="space-y-2">
                    <Label
                      htmlFor={`valor-${forma.id}`}
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        handleValorChange(forma.id, e.target.value)
                      }
                    />
                  </div>
                );
          ...
          <CardContent className="p-4 md:p-6 pt-0 space-y-3">
            {notificacoesPendentes.map((notif) => {
              ...
              const dadosConferencia = {
                ...
                valores: Object.entries(valoresObj).reduce(
