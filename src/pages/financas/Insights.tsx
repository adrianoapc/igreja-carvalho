    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <Select
            value={periodo}
            onValueChange={(v) => setPeriodo(v as "3" | "6" | "12")}
          >
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-3">
                  {insights.frequenciaFornecedores
                    .slice(0, 8)
                    .map((item, index) => (
                      <div
                        key={index}
                ...
                <ResponsiveContainer width="100%" height={400}>
                  <PieChart>
                    <Pie
                      ...
                      label={(entry) =>
                        `${entry.nome}: ${(
                          (entry.total / insights.totalGasto) *
                          100
                        ).toFixed(1)}%`
                      }
                    >
                    ...
                    <div key={index} className="space-y-1">
                      ...
                      <div className="h-2 bg-muted rounded-full overflow-hidden">
                        <div
                          ...
                          style={{
                            width: `${
                              (cat.total / insights.totalGasto) * 100
                            }%`,
                          }}
                        />
                      </div>
                      <div className="flex justify-between text-xs text-muted-foreground">
                        ...
                        <span>
                          {((cat.total / insights.totalGasto) * 100).toFixed(1)}
                          %
                        </span>
                      </div>
                    </div>
        ...
        <TabsContent value="tendencias" className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-lg font-semibold">
                  {insights.mesMaiorGasto?.mes}
                </p>
                <p className="text-2xl font-bold text-primary">
                  {insights.mesMaiorGasto
                    ? formatValue(insights.mesMaiorGasto.total)
                    : "-"}
                </p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <p className="text-lg font-semibold">
                  {insights.mesMenorGasto?.mes}
                </p>
                <p className="text-2xl font-bold text-green-600">
                  {insights.mesMenorGasto
                    ? formatValue(insights.mesMenorGasto.total)
                    : "-"}
                </p>
              </CardContent>
            </Card>
        ...
        <TabsContent value="anomalias" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Valores significativamente acima da média (
                {formatValue(insights.mediaTransacao)})
              </CardDescription>
            </CardHeader>
            <CardContent>
              {insights.anomalias.length === 0 ? (
                ...
                <div className="space-y-3">
                  {insights.anomalias.map((anomalia, index) => (
                    <div
                      key={index}
                      className="flex items-start gap-3 p-4 border rounded-lg bg-destructive/5"
                    >
...
function processarInsights(transacoes: TransacaoFinanceira[]) {
  ...
  const fornecedoresMap = new Map<
    string,
    { nome: string; total: number; transacoes: number; valores: number[] }
  >();
  transacoes.forEach((t) => {
    const nome = t.fornecedor?.nome || "Sem fornecedor";
    const current = fornecedoresMap.get(nome) || {
      nome,
      total: 0,
      transacoes: 0,
      valores: [],
  ...
  const topFornecedores = Array.from(fornecedoresMap.values()).sort(
    (a, b) => b.total - a.total
  );

  const frequenciaFornecedores = topFornecedores
    .map((f) => ({
      nome: f.nome,
  ...
  const categoriasMap = new Map<
    string,
    { nome: string; total: number; transacoes: number }
  >();
  transacoes.forEach((t) => {
    const nome = t.categoria?.nome || "Sem categoria";
    const current = categoriasMap.get(nome) || {
      nome,
      total: 0,
      transacoes: 0,
    };
  ...
  const mesMaiorGasto = gastosMensais.reduce(
    (max, curr) => (curr.total > max.total ? curr : max),
    gastosMensais[0]
  );
  const mesMenorGasto = gastosMensais.reduce(
  ...
  const primeiraMetade =
    gastosMensais.slice(0, metade).reduce((acc, m) => acc + m.total, 0) /
    metade;
  const segundaMetade =
    gastosMensais.slice(metade).reduce((acc, m) => acc + m.total, 0) /
    (gastosMensais.length - metade);
