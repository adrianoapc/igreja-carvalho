...
export default function Financas() {
  ...
  const totalEmCaixa =
    contas?.reduce((sum, conta) => sum + Number(conta.saldo_atual), 0) || 0;
  const totalEntradasMes =
    transacoesEntrada?.reduce((sum, t) => sum + Number(t.valor), 0) || 0;
  const totalSaidasMes =
    transacoesSaida?.reduce((sum, t) => sum + Number(t.valor), 0) || 0;
          ...
          >
            <CardContent className="p-5">
              ...
              <h3 className="font-semibold text-base mb-1">
                Conferência de Ofertas
              </h3>
              <p className="text-xs text-muted-foreground">
                Registre e envie registros para conferência
              </p>
            </CardContent>
          </Card>
            ...
            <CardContent className="p-5">
              ...
              {reembolsosPendentes && reembolsosPendentes.length > 0 && (
                <Badge variant="destructive" className="text-xs">
                  {reembolsosPendentes.length} pendente
                  {reembolsosPendentes.length !== 1 ? "s" : ""}
                </Badge>
              )}
            </CardContent>
          </Card>
          ...
          <CardContent className="p-5 flex flex-col gap-3">
            <div className="flex items-start justify-between gap-3">
              <div className="flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-lg font-bold">
                    {formatCurrency(contasSemanaTotal)}
                  </p>
                  <p className="text-xs text-muted-foreground">
                  ...
                  {contasVencidas.length > 0 && (
                    <p className="text-xs text-destructive mt-1">
                      {contasVencidas.length} vencida
                      {contasVencidas.length !== 1 ? "s" : ""}. Priorize hoje.
                    </p>
                  )}
                </div>
              </div>
              <Button
                ...
                size="sm"
                onClick={() =>
                  navigate("/financas/saidas?status=pendente&range=semana")
                }
              >
                Ver detalhes
                <ArrowRight className="w-4 h-4 ml-2" />
              ...
              return (
                ...
                >
                  <CardContent className="p-4">
                    <div
                      className={`p-3 rounded-lg ${module.color} w-fit mb-3`}
                    >
