  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      columnNames.forEach(col => {
        ...
        if (colLower.includes('subcat') || colLower.includes('sub-categ')) autoMapping.subcategoria = col;
        if (colLower.includes('centro') || colLower.includes('custo')) autoMapping.centro_custo = col;
        if (colLower.includes('ministerial') || colLower.includes('base')) autoMapping.base_ministerial = col;
        if (colLower.includes('forma') || colLower.includes('pagamento')) autoMapping.forma_pagamento = col;
  ...
  const processarImportacao = async () => {
    ...
    try {
      ...
      for (let i = 0; i < preview.length; i++) {
        ...
        try {
          ...
          if (mapping.subcategoria && subcategorias) {
            ...
            const subEncontrada = subcategorias.find(s => 
          ...
          if (mapping.centro_custo && centrosCusto) {
            ...
            const ccEncontrado = centrosCusto.find(c => 
          ...
          if (mapping.base_ministerial && basesMinisteriais) {
            ...
            const bmEncontrada = basesMinisteriais.find(b => 
              (b.titulo || "").toLowerCase() === String(nomeBM).toLowerCase().trim()
            );
          ...
          if (mapping.forma_pagamento && formasPagamento) {
            ...
            const fpEncontrada = formasPagamento.find(fp => 
      ...
      for (let i = 0; i < transacoes.length; i += chunkSize) {
        ...
        setImportProgress(Math.min(100, Math.round(((i + chunk.length) / transacoes.length) * 100)));
      }
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={mapping.subcategoria} onValueChange={(v) => setMapping({...mapping, subcategoria: v})}>
                      <SelectTrigger className="h-9">
                        <SelectValue placeholder="Selecione a coluna" />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={mapping.centro_custo} onValueChange={(v) => setMapping({...mapping, centro_custo: v})}>
                      <SelectTrigger className="h-9">
                        <SelectValue placeholder="Selecione a coluna" />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={mapping.base_ministerial} onValueChange={(v) => setMapping({...mapping, base_ministerial: v})}>
                      <SelectTrigger className="h-9">
                        <SelectValue placeholder="Selecione a coluna" />
                      </SelectTrigger>
                      <SelectContent>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={mapping.forma_pagamento} onValueChange={(v) => setMapping({...mapping, forma_pagamento: v})}>
                      <SelectTrigger className="h-9">
                        <SelectValue placeholder="Selecione a coluna" />
                      </SelectTrigger>
                      <SelectContent>
        ...
        <div className="border-t bg-muted/50 px-4 py-3 md:px-6 flex gap-2 justify-end">
          {loading && (
            <div className="flex items-center gap-2 mr-auto w-full max-w-xs">
              ...
              <span className="text-xs text-muted-foreground whitespace-nowrap">{importProgress}%</span>
            </div>
          )}
          <Button
            variant="outline"
