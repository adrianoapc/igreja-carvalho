...
export function PedidoDetailsDialog({
  ...
  const [observacoes, setObservacoes] = React.useState(
    pedido?.observacoes_intercessor || ""
  );
  ...
  React.useEffect(() => {
    if (pedido) {
      ...
      setObservacoes(pedido.observacoes_intercessor || "");
    }
  }, [pedido]);
            ...
            <div className="flex items-center gap-4 text-sm text-muted-foreground">
              ...
              {pedido.data_alocacao && (
                <div className="flex items-center gap-1">
                  <span>
                    Alocado em:{" "}
                    {new Date(pedido.data_alocacao).toLocaleDateString("pt-BR")}
                  </span>
                </div>
              )}
              {pedido.data_resposta && (
                <div className="flex items-center gap-1">
                  <span>
                    Respondido em:{" "}
                    {new Date(pedido.data_resposta).toLocaleDateString("pt-BR")}
                  </span>
                </div>
              )}
            </div>
