    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className={`p-3 rounded-lg ${stats?.tarefasAtrasadas ? "bg-destructive/10" : "bg-muted/50"}`}>
            <div className="flex items-center gap-2">
              <AlertTriangle className={`w-4 h-4 ${stats?.tarefasAtrasadas ? "text-destructive" : "text-muted-foreground"}`} />
              <span className="text-xs text-muted-foreground">Tarefas Atrasadas</span>
            </div>
            <p className={`text-2xl font-bold mt-1 ${stats?.tarefasAtrasadas ? "text-destructive" : "text-foreground"}`}>
          ...
          <div className="p-3 rounded-lg bg-muted/50">
            ...
            <p className="text-2xl font-bold mt-1 text-foreground">{stats?.projetosAtivos || 0}</p>
          </div>
        </div>
        ...
        {stats?.topProjetos && stats.topProjetos.length > 0 && (
          <div className="space-y-3">
            <h4 className="text-xs font-medium text-muted-foreground uppercase">Top Projetos</h4>
            {stats.topProjetos.map(projeto => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-sm font-medium text-foreground truncate">{projeto.titulo}</span>
