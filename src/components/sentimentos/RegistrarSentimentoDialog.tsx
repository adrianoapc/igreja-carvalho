...
export default function RegistrarSentimentoDialog({
  open,
  onOpenChange,
}: RegistrarSentimentoDialogProps) {
  const { toast } = useToast();
  const navigate = useNavigate();
  const { igrejaId, loading: igrejaLoading } = useIgrejaId();
  const [step, setStep] = useState<
    "select" | "message" | "result" | "already_registered"
  >("select");
  const [selectedSentimento, setSelectedSentimento] =
    useState<SentimentoTipo | null>(null);
  const [mensagem, setMensagem] = useState("");
  const [loading, setLoading] = useState(false);
  const [existingSentimento, setExistingSentimento] =
    useState<SentimentoTipo | null>(null);
        ...
        setTimeout(() => {
          if (igrejaId) {
            ...
            toast({
              title: "Erro",
              description:
                "Não foi possível identificar sua igreja. Tente novamente.",
              variant: "destructive",
            });
            setLoading(false);
          }
  ...
  const saveSentimento = async (sentimento: SentimentoTipo, msg: string) => {
    try {
      ...
      if (!igrejaId) {
        throw new Error(
          "Igreja não identificada. Tente novamente em alguns instantes."
        );
      }
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Não foi possível registrar seu sentimento.";
      toast({
        title: "Erro",
  ...
  const renderAlreadyRegistered = () => {
    const config = existingSentimento
      ? sentimentosConfig[existingSentimento]
      : null;
    return (
      ...
      return (
        <div className="text-center space-y-4 py-6">
          ...
          <p className="text-muted-foreground">
            Agradecemos por compartilhar sua alegria conosco. Que Deus continue
            abençoando sua vida!
          </p>
          <Button onClick={handleClose} className="w-full">
            Fechar
    ...
    if (config.type === "grateful") {
      return (
        <div className="text-center space-y-4 py-6">
          ...
          <p className="text-muted-foreground">
            Que bênção! Gostaríamos de convidar você a compartilhar seu
            testemunho para edificar outros irmãos.
          </p>
    ...
    if (config.type === "negative") {
      return (
        <div className="text-center space-y-4 py-6">
          ...
          <p className="text-muted-foreground">
            Sentimos muito que esteja passando por isso. Gostaríamos de orar por
            você. Que tal compartilhar um pedido de oração com nossa equipe de
            intercessão?
          </p>
          <div className="space-y-2">
            <Button
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 md:px-6">
          {step === "select" && (
            <div className="grid grid-cols-3 gap-3">
              {(
                Object.entries(sentimentosConfig) as [
                  SentimentoTipo,
                  SentimentoConfig
                ][]
              ).map(([key, config]) => (
            ...
            <div className="space-y-4">
              <div className="text-center">
                <span className="text-6xl">
                  {sentimentosConfig[selectedSentimento].emoji}
                </span>
              </div>
              <Textarea
