...
export function MidiaDialog({
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (arquivo) {
        ...
        const {
          data: { publicUrl },
        } = supabase.storage.from("midias").getPublicUrl(filePath);
      ...
      if (midia) {
        ...
        const successMessage = scheduledAt
          ? `Mídia atualizada! Será publicada em ${format(
              scheduledAt,
              "dd/MM/yyyy 'às' HH:mm",
              { locale: ptBR }
            )}`
        ...
        const successMessage = scheduledAt
          ? `Mídia adicionada! Será publicada em ${format(
              scheduledAt,
              "dd/MM/yyyy 'às' HH:mm",
              { locale: ptBR }
            )}`
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : String(error) || "Erro ao salvar mídia"
      );
    } finally {
      setSaving(false);
      setUploading(false);
  ...
  const atualizarTags = async (midiaId: string) => {
    ...
    if (tagsSelecionadas.length > 0) {
      ...
      await supabase.from("midia_tags").insert(tagsData);
    }
  };

  // Função para sugerir tags automaticamente baseada em tipo e canal
  const sugerirTagsPorTipoCanal = (
    tipoMidia: string,
    canalMidia: string
  ): string[] => {
